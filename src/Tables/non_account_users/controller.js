...
import NonAccountUser from './schema.js'
import { v4 as Uuidv4 } from 'uuid';



function processReturnForm(req, res) {
    console.log('Processing Return Form')
    const user = new NonAccountUser(req);
    const connection_request_from = req.body.connection_request_from
    const connection_id = Uuidv4()
    console.log(user)
    Pool.query(Queries.check_if_phone_number_exists, [user.phone_number], (error, query_results) => {
        if (error) throw error
        if (query_results.rows.length) {
            res.send("Phone Number already exists.")
        } else {
            //if new phone number, then add new user
            Pool.query(Queries.add_user, [user.non_acc_id, user.first_name, user.last_name, user.phone_number_country, user.phone_number], (error, query_results) => {
                if (error) throw error;
                console.log("User created")
            })
            Pool.query(Queries.add_user_connection, [connection_id, connection_request_from, user.non_acc_id], (error, query_results) => {
                if (error) throw error;
...
function getUserConnectionById(req, res) {
    console.log('getting user by id')
    const user_id = "1234";
    Pool.query(Queries.get_user_connection_by_id, [user_id], (error, query_results) => {
        if (error) throw error;
